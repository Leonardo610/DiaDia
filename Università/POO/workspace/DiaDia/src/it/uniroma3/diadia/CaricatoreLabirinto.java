...
public class CaricatoreLabirinto {
	...
	public CaricatoreLabirinto(Reader lettore) {
		...
		this.reader = new LineNumberReader(lettore); 
	}
	...
	private void leggiECollocaPersonaggi() throws FormatoFileNonValidoException {
		...
		String presPersonaggio = null;
		try (Scanner scannerLinea = new Scanner(leggiPersonaggi)) {
			check(scannerLinea.hasNext(), msgTerminazionePrecoce("il tipo del personaggio."));
			tipoPersonaggio = scannerLinea.next();
			check(scannerLinea.hasNext(), msgTerminazionePrecoce("il nome del personaggio " + tipoPersonaggio));
			nomePersonaggio = scannerLinea.next();
			check(scannerLinea.hasNext(), msgTerminazionePrecoce("la presentazione del personaggio " + tipoPersonaggio));
			presPersonaggio = scannerLinea.next();
			check(scannerLinea.hasNext(), msgTerminazionePrecoce("il nome della stanza in cui collocare il personaggio " + tipoPersonaggio + "."));
			nomeStanza = scannerLinea.next();
		}
		collocaPersonaggio(tipoPersonaggio, nomePersonaggio, presPersonaggio, nomeStanza);
	}
	
	private void collocaPersonaggio(String tipoPersonaggio, String nomePersonaggio, String presPersonaggio, String nomeStanza){
		try {
			AbstractPersonaggio personaggio = (AbstractPersonaggio) Class.forName("it.uniroma3.diadia.personaggi." + tipoPersonaggio).newInstance();
			...
			personaggio.setPresentazione(presPersonaggio);
